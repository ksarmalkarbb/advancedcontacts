...
- (NSNumber *)hasContactsPermissions {
    CNAuthorizationStatus authorizationStatus = [CNContactStore authorizationStatusForEntityType:CNEntityTypeContacts];

        return @(authorizationStatus == CNAuthorizationStatusAuthorized);
}

- (void)requestContactsPermissions:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
    [self.contactsStore requestAccessForEntityType:CNEntityTypeContacts completionHandler:^(BOOL granted, NSError * _Nullable error) {
            if (error) {
                return reject(@"Error", @"permissison not granted", error);
            }
